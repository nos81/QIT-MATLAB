...
%% Lindblad superoperators, fixed point states

A = rand_GL(dim);
A(abs(A)<2.3) = 0;  % add some sparsity
L = superop_lindblad({A}, H);
[Q, s] = superop_fp(L);

assert_o(norm(L*Q), 0, tol);  % columns of Q belong in ker L
assert_o(trace(inv_vec(Q(:,1))), 1, tol);  % first col is trace-1
for k=1:size(Q,2)
    B = inv_vec(Q(:,k));
    assert_o(norm(B-B'), 0, tol);  % all cols are hermitian
end
