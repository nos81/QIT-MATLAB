...
% OP_LIST  Operator from connection list.
%  O = op_list(G, dim)
%
%  Returns the operator O defined by the connection list G.
%  dim is a vector of subsystem dimensions for O.
%
%  G is a cell vector of cell arrays, G = {c1, c2, ..., cn},
%  where each cell array ci corresponds to a term in O.
%
%  An array that has 2 columns and k rows, ci = {A1, s1; A2, s2; ... ; Ak, sk},
%  where Aj are operators and sj subsystem indices, corresponds to the
%  term given by the tensor product
%
...
%  each subsystem corresponds to a vertex and each array ci in the list
...
%
%  corresponds to the (e.g. Hamiltonian) operator
...
for k=1:n
  ...
  for j=1:s(1)
    ...
    if (b < a)
      error('Spec %d not in nondescending order.', k)
    end
